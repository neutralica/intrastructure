...
export type FullJSONRequest<T = unknown> =
    & URLSpec
    & HeaderSpec
    & QuerySpec
    & BodySpec<T>
...
export type ValidNetRequest<S> =
    S extends NetRequestTEMP
    ? S
    : never;


export interface RequestBuilder<S> {
    // requireMethod(m: N_ReqMethod): N_request;
    ...
    // this maybe wrong since it needs to account for both JSON and HTML disc
    ...
    // body generic is probably mistyped here
    ...
    // NOT CORRECT: TO-DO later if possible when JSON and HTML are correct
